        ...
        echo '<a href="/photos/PhotosByCatalog/'.$catalog['Id'].'/'.$catalog['UserId'].' ">';
            ...
            echo '<div class="row">';
                ...
                if (UserDetails::isLogged()){
                    echo '<div class="col-md-4 col-xs-12 col-lg-4"><a class="btn btn-success btnCatalogComments" style="cursor:pointer;" onclick="addComent('.$catalog['Id'].')">Добави коментар</a></div>';
                }
                echo '<div class="col-md-4 col-xs-12 col-lg-4">'
                    . '<a style="cursor:pointer;" class="btn btn-info btnCatalogComments" id="showComment' . $catalog['Id'] . '" onclick="showComment(' . $catalog['Id'] . ')">Покажи коментарите</a>'
                    . '<a style="display:none;" class="btn btn-danger btnCatalogComments" id="hideComment' . $catalog['Id'] . '" onclick="hideComment(' . $catalog['Id'] . ')" >Скрий коментарите</a>'
                . '</div>';
                $idUserLike = FALSE;
                if (isset($this->likes)) {
                    ...
                    if ($idUserLike == TRUE) {
                        ...
                        . '<a style="cursor:pointer; display:none;" class="btn btn-default btnCatalogComments" id="likeCatalog'.$catalog['Id'].'" onclick="likeCatalog(' . $catalog['Id'] . ')">Харесва ми</a>'
                        . '<a style="cursor:pointer;" class="btn btn-default btnCatalogComments" id="unLikeCatalog'.$catalog['Id'].'" onclick="unLikeCatalog(' . $catalog['Id'] . ')">Вече не харесвам</a>';
                        echo '</div>';
                    } else{
                        echo '<div class="col-md-2 col-xs-12 col-lg-2">'
                        . '<a style="cursor:pointer;" class="btn btn-default btnCatalogComments" id="likeCatalog'.$catalog['Id'].'" onclick="likeCatalog(' . $catalog['Id'] . ')">Харесва ми</a>'
                        . '<a style="cursor:pointer;  display:none;" class="btn btn-default btnCatalogComments" id="unLikeCatalog'.$catalog['Id'].'" onclick="unLikeCatalog(' . $catalog['Id'] . ')">Вече не харесвам</a>';
                        echo '</div>';
                    }
                }
                  ...
                  if ($comment['CatalogId'] == $catalog['Id']) {
                      ...
                      if (UserDetails::isLogged() && $comment['UserId'] == UserDetails::getUserId()) {
                        echo '<div class="col-md-2 col-xs-2 col-lg-2">';
                             echo '<span style="cursor:pointer;" class="iconsCatalog"><i onclick="editComment(' . $comment['Id'] . ', ' . $comment['UserId'] . ', \'' . str_replace("'", "", htmlspecialchars($comment['Description']))  . '\')" title="редакция" class="fa fa-pencil rightSpase"></i>';
                             echo '<i onclick="deleteComment(' . $comment['Id'] . ', ' . $comment['UserId'] . ')" title="изтриване" class="fa fa-trash-o rightSpase"></i></span>' ;
                        echo '</div>'; 
                      }
                  }
              }
